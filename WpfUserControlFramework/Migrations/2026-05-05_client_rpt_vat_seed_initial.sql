...
--   - One row per day per (active channel x active user role x demo tax band).
...
WITH params AS (
    -- Client: set exactly three `public.taxes.tax_id` values for demo VAT bands (must exist; typical seed = 1, 2, 3).
    SELECT
        1 AS demo_tax_id_1,
        2 AS demo_tax_id_2,
        3 AS demo_tax_id_3
